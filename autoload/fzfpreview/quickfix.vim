...
function! s:error_handler(errs) abort
    ...
    normal! m'
    let cmd = fzfpreview#action_for(a:errs[0])
...
function! fzfpreview#quickfix#run(loc, bang) abort
    let expect_keys = join(keys(get(g:, 'fzf_action', fzfpreview#get_default_action())), ',')
    call fzf#run(fzf#wrap(a:loc ? 'loclist' : 'quickfix', fzfpreview#p(a:bang, {
